...
(function () {
   ...
   */
  /**
   * An employee's OWN (individual) allocation in a phase → { office, zone, label }
   * or null. Deliberately ignores bulk team allocations — those are team-level.
   */
  function individualPlacement(scenario, emp, phase) {
    var allocs = (scenario.allocations || []).filter(function (a) {
      return a.type === C.ALLOCATION_TYPE.EMPLOYEE && a.employeeId === emp.id;
    });
    for (var i = 0; i < allocs.length; i++) {
      var o = U.findById(scenario.offices, allocs[i].targetOfficeId);
      if (!o) { continue; }
      ...
      return { office: o, zone: z, label: o.name + (z ? ' / ' + z.name : '') };
    }
    return null;
  }
  ...
  function renderMembers(scenario, team, viewOnly) {
    ...
    if (namedCount > 0) {
      members.forEach(function (emp) {
        // Per-member office is shown ONLY from the member's OWN (individual)
        // allocation. Bulk team seats are team-level and must not be attributed
        // to individual members (they show "—" until named-placed).
        var asIsInd = individualPlacement(scenario, emp, C.OFFICE_PHASE.ASIS);
        var tobeInd = individualPlacement(scenario, emp, C.OFFICE_PHASE.TOBE);
        var asisText = asIsInd ? asIsInd.label : '—';
        var tobeText = tobeInd ? tobeInd.label : '—';
  ...
  /**
   * A team's actual placement in a phase -> [{officeId, zoneId, count}], grouped
   * by office/zone. Aggregates BOTH bulk TEAM allocations and individual named-
   * employee allocations (count = max(teamSeats, namedCount) per office/zone),
   * matching the Teams-table columns and the occupancy calc. Editing these rows
   * sets the bulk team seats; named employees remain placed (they dedupe by max).
   */
  function teamPhaseRows(scenario, team, phase) {
    ...
    (scenario.allocations || []).forEach(function (a) {
      ...
      var zoneId = a.targetZoneId || '';
      var key = a.targetOfficeId + '|' + zoneId;
      if (!byKey[key]) { byKey[key] = { officeId: a.targetOfficeId, zoneId: a.targetZoneId || null, teamSeats: 0, namedCount: 0 }; order.push(key); }
      if (a.type === C.ALLOCATION_TYPE.TEAM) {
        byKey[key].teamSeats += (a.employeesCount || 0);
      } else if (a.type === C.ALLOCATION_TYPE.EMPLOYEE && a.employeeId) {
        byKey[key].namedCount += 1;
    ...
    return order.map(function (k) {
      var e = byKey[k];
      return { officeId: e.officeId, zoneId: e.zoneId, count: Math.max(e.teamSeats, e.namedCount) };
    });
  }
