...
App.cfEdit = (function () {
  ...
  // A CF row's identity = name + phase.
  function rowIdentity(r) { return (r.name || '').trim().toLowerCase() + '|' + (r.phase || ''); }
  var NO_TENANT_KEY = 'без арендатора';
  // Names the CF computation itself produces, so they must NOT be preserved as
  // "manual" on reset (else a stale office/tenant/"Без арендатора" row would be
  // kept AND recomputed → duplicates). Keyed by name+phase, from the scenario's
  // offices/tenants (identity — not row id — because imports give all rows
  // 'cfrow_' ids).
  function computedOfficeKeys(scenario) {
    var set = {};
    (scenario.offices || []).forEach(function (o) {
      if (o.type === C.OFFICE_TYPE.PHYSICAL) { set[(o.name || '').trim().toLowerCase() + '|' + (o.phase || '')] = true; }
    });
    return set;
  }
  function computedTenantKeys(scenario) {
    var set = {};
    set[NO_TENANT_KEY + '|' + C.OFFICE_PHASE.ASIS] = true;
    set[NO_TENANT_KEY + '|' + C.OFFICE_PHASE.TOBE] = true;
    (scenario.offices || []).forEach(function (o) {
      if (o.type !== C.OFFICE_TYPE.PHYSICAL) { return; }
      (o.tenants || []).forEach(function (t) { set[(t.name || '').trim().toLowerCase() + '|' + (o.phase || '')] = true; });
    });
    return set;
  }
  // Rows the user genuinely added — names that no office/tenant/"Без арендатора" produces.
  function extraRows(prev, knownKeys) {
    return (prev || []).filter(function (r) { return !knownKeys[rowIdentity(r)]; });
  }

  /**
   * "Пересчитать": recompute the office/card-derived CF rows from the current
   * offices, preserving only rows the user genuinely added (names not produced
   * by any office/tenant, incl. "Без арендатора"). Nothing extra → drop the
   * override entirely (fully live recompute).
   */
  function reset(scenario) {
    ...
    state.commit('Пересчёт CF из офисов', function () {
      ...
      var extraOffices = extraRows(prevOffices, computedOfficeKeys(scenario));
      var extraTenants = extraRows(prevTenants, computedTenantKeys(scenario));
